...
func main() {
	...
	if err := login.DoLogin(ctx, NEWSPICK_ID, NEWSPICK_PW, NEWSPICK_URL); err != nil {
		log.Fatal(err)
		return
	}
	profitItems, _ := ScrapeProfitNews(ctx, 10)

	for i := range profitItems {
		fmt.Printf("%v, %v\n %v\n", profitItems[i].Title, profitItems[i].Image, profitItems[i].Link)
	}
}

func ScrapeProfitNews(ctx context.Context, max int) ([]ProfitItem, error) {
	// 섹션 로딩 대기
	...
	items := make([]ProfitItem, 0, len(lis))
